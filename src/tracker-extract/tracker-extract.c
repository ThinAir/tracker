                   ...
                   TrackerExtractInfo **info_out)
{
    ...
    *info_out = NULL;
    ...
    info = tracker_extract_info_new_with_cb (file, task->mimetype,task->parsed_data_availble_cb,task->context);
    g_object_unref (file);
    ...
    if (task->mimetype && *task->mimetype) {
        ...
        printf("getting meta data  failed...");
        return FALSE;
    }
     ...
     */
    //printf("getting meta data  for mime... %s",mime_used);
    if (mime_used) {
        if (task->cur_func) {
//            printf ("Using %s...",
//                     task->cur_module ?
                  ...
                  GError        **error)
{
    TrackerExtractTask *task;
    gchar *mimetype_used;
    //printf("extraction data for uri %s \n",uri);
    if (!mimetype || !*mimetype) {
        ...
        const gchar *mime_used = NULL;
        gchar *file_path =  g_file_get_path (file);
        mime_used = magic_file(magic_cookie,  file_path);
        mimetype_used = g_strdup (mime_used);
        magic_close(magic_cookie);
        g_object_unref (file);
        g_free(file_path);
        
        g_message ("MIME type guessed as last '%s' (from GIO)", mimetype_used);
        //printf ("MIME type guessed as last'%s' (from GIO)", mimetype_used);
    ...
    g_message ("MIME type guessed done as last '%s' (from GIO)", mimetype_used);
    //printf ("MIME type guessed as done  last'%s' (from GIO)", mimetype_used);
    ...
    if (task->cancellable) {
        //printf ("cancellable block");
        g_message ("cancellable block");
        task->signal_id = g_cancellable_connect (cancellable,
                                                 G_CALLBACK (task_cancellable_cancelled_cb),
                                                 task, NULL);
    }
    //printf ("returnig task...");
    g_message ("returnig task...");
               ...
               GModule        *module)
{
    ...
    //printf("filtering module step 1 ****\n");

    if (!module) {
        return FALSE;
    }
    //printf("filtering module step 2 ****\n");
                      ...
                      gpointer             user_data)
{
    ...
    task->parsed_data_availble_cb= NULL;
    task->context= NULL;
    if (error) {
        printf ("Could not get mimetype, %s", error->message);
...
/* This function can be called in any thread */
void
tracker_extract_file_with_cb (TrackerExtract         *extract,
                              const gchar            *file,
                              const gchar            *mimetype,
                              GCancellable           *cancellable,
                              GAsyncReadyCallback     cb,
                              ...
                              gpointer                user_data)
{
    ...
    task = extract_task_new (extract, file, mimetype, cancellable,
                             G_ASYNC_RESULT (async_task), &error);
    task->parsed_data_availble_cb= parsed_data_availble_cb;
...
{
    //printf("called from commandline\n");
    GError *error = NULL;
    TrackerExtractPrivate *priv;
    TrackerExtractTask *task;
    TrackerExtractInfo *info;
